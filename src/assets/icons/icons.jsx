export const Info = () => (
  <lord-icon
    src="https://cdn.lordicon.com/dxoycpzg.json"
    trigger="morph"
    style={{ width: 50, height: 50, cursor: "pointer" }}
  ></lord-icon>
);
export const Cart = () => (
  <lord-icon
    src="https://cdn.lordicon.com/rmzhcgbh.json"
    trigger="hover"
    colors="primary:#ffc738,secondary:#b4b4b4,tertiary:#f24c00"
    style={{ width: 50, height: 50 }}
  ></lord-icon>
);

export const AddCart = () => (
  <lord-icon
    src="https://cdn.lordicon.com/cllunfud.json"
    trigger="click"
    style={{ width: 50, height: 50 }}
  ></lord-icon>
);

export const Plus = () => (
  <lord-icon
    src="https://cdn.lordicon.com/ejxwvtlg.json"
    trigger="click"
    colors="outline:#ffffff,primary:#ffffff,secondary:#16c72e"
    style={{ width: 50, height: 50 }}
  ></lord-icon>
);

export const Minus = () => (
  <lord-icon
    src="https://cdn.lordicon.com/ricosjfq.json"
    trigger="click"
    colors="outline:#ffffff,primary:#ffffff,secondary:#c71f16"
    style={{ width: 50, height: 50 }}
  ></lord-icon>
);

export const Trash = () => (
  <lord-icon
    src="https://cdn.lordicon.com/exkbusmy.json"
    trigger="hover"
    colors="outline:#ffffff,primary:#ffffff,secondary:#c71f16,tertiary:#ffffff"
    style={{ width: 50, height: 50 }}
  ></lord-icon>
);

export const ButtonClose = () => (
  <div className="noselect">
    <span className="text">Fechar</span>
    <span className="icon">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width={24}
        height={24}
        viewBox="0 0 24 24"
      >
        <path d="M24 20.188l-8.315-8.209 8.2-8.282-3.697-3.697-8.212 8.318-8.31-8.203-3.666 3.666 8.321 8.24-8.206 8.313 3.666 3.666 8.237-8.318 8.285 8.203z" />
      </svg>
    </span>
  </div>
);
